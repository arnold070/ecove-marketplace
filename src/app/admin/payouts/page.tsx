...
export default function AdminPayoutsPage() {
  ...
  <>
    ...
    <PromptModal
      ...
      label={
        modal?.act === 'mark_paid'
          ? `Bank transfer reference for ₦${modal?.amount} to ${modal?.vendor}:`
          : 'Reason for holding this payout:'
      }
      placeholder={
        modal?.act === 'mark_paid'
          ? 'e.g. TRF2024123456789'
          : 'e.g. Awaiting verification…'
      }
      confirmLabel={modal?.act === 'mark_paid' ? 'Mark as Paid' : 'Hold Payout'}
      danger={modal?.act === 'reject'}
      onConfirm={(val) => {
        if (modal) {
          if (modal.act === 'mark_paid') {
            action.mutate({ id: modal.id, act: modal.act, ref: val })
          } else {
            action.mutate({ id: modal.id, act: modal.act, note: val })
          }
        }
        setModal(null)
      }}
